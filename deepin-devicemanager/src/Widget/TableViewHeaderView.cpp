...
void TableViewHeaderView::paintSection(QPainter *painter, const QRect &rect, int logicalIndex) const
{
    painter->save();
...
void TableViewHeaderView::paintEvent(QPaintEvent *event)
{
...
QSize TableViewHeaderView::sizeHint() const
{
    return QSize(width(), 36 + m_spacing);
}

int TableViewHeaderView::sectionSizeHint(int logicalIndex) const
{
    QStyleOptionHeader option;
    initStyleOption(&option);
    DStyle *style = dynamic_cast<DStyle *>(DApplication::style());
