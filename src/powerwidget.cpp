...
PowerWidget::PowerWidget(QWidget *parent, bool &success_init_on_loonson):DeviceInfoWidgetBase (parent,tr("Power"))
{
    QString os_info = DeviceInfoParser::Instance().getOsInfo();
    if (os_info.contains("loongson",Qt::CaseInsensitive )) {
        success_init_on_loonson = initOnLoonsonDesktop();
    }
    else {
        initWidget();
        success_init_on_loonson = true;
    }
}
...
bool PowerWidget::initOnLoonsonDesktop()
{
    QStringList lshwSwitchingpowerList = DeviceInfoParser::Instance().getLshwSwitchingpowerList();
    QStringList demidecodeSwitchingpowerList = DeviceInfoParser::Instance().getDemidecodeSwitchingpowerList();
    QStringList upowerSwitchingList = DeviceInfoParser::Instance().getUpowerSwitchingList();
