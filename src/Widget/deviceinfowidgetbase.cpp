...
void DeviceInfoWidgetBase::initWidget()
{
...
void DeviceInfoWidgetBase::toHtmlString(QDomDocument &doc, const DeviceBase &di)
{
...
QTextStream &operator<<(QTextStream &ds, const DeviceBase &di)
{
    QList<DeviceComputer> devices = DeviceManager::instance()->getComputerDevices();
    DeviceComputer device;
    if (devices.size() > 0) {
...
bool writeDeviceInfoToDoc(const DeviceBase &di, Docx::Document &doc)
{
    QList<DeviceComputer> devices = DeviceManager::instance()->getComputerDevices();
    DeviceComputer device;
    if (devices.size() > 0) {
...
bool writeDeviceInfoToXls(const DeviceBase &di, QXlsx::Document &xlsx)
{
    QList<DeviceComputer> devices = DeviceManager::instance()->getComputerDevices();
    DeviceComputer device;
    if (devices.size() > 0) {
