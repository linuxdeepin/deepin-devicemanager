...
InlineShape *InlineShapes::addPicture(const QPair<ImagePart *, QString> &imagePartAndId, Run *run)
{
    ...
    CT_Picture *pic = new CT_Picture(m_part->m_dom, QStringLiteral("0"), imagePartAndId.first->fileName(), imagePartAndId.second, imagePartAndId.first->defaultCx().emu(), imagePartAndId.first->defaultCy().emu());

    CT_Inline *lnLine = new CT_Inline(m_part->m_dom, imagePartAndId.first->defaultCx().emu(), imagePartAndId.first->defaultCy().emu(), QString::number(shapeId), pic);
