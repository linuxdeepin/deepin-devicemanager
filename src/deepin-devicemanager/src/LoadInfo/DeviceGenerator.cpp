...
void DeviceGenerator::getImageInfoFromHwinfo()
{
    ...
    for (; it != lstMap.end(); ++it) {
        ...
        if ((*it)["Model"].contains("camera", Qt::CaseInsensitive) || (*it)["Device"].contains("camera", Qt::CaseInsensitive) || (*it)["Driver"].contains("uvcvideo", Qt::CaseInsensitive) || (*it)["Model"].contains("webcam", Qt::CaseInsensitive)) { // "webcam"  bug39981)
